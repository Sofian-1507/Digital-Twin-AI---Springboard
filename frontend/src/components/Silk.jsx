...
// kept close to upstream for easy future updates. Only the default `color` was changed
// to match this app's teal token (--color-indigo-600 in index.css).
...
const Silk = ({
  ...
  // Three.js shader uniforms are inherently mutable-in-place (that's the whole
  // API — a material reads `.value` fresh every frame, it isn't a React
  // render input), which the eslint-plugin-react-hooks v7 purity/immutability
  // rules can't model: they assume anything created by a hook, or passed as
  // a prop, is never written to again. This mutation is safe because the
  // uniforms object never feeds back into React state or JSX output — it's
  // an opaque handle Three.js reads on its own render loop.
  const uniforms = useMemo(
    ...
    // Intentionally created once and mutated thereafter (see comment above),
    // not recomputed per prop change.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    []
  );

  useEffect(() => {
    // eslint-disable-next-line react-hooks/immutability -- see comment above uniforms
