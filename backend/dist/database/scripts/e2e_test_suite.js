...
exports.runE2ETestSuite = runE2ETestSuite;
...
const finance_repository_1 = require("../repositories/finance_repository");
const study_repository_1 = require("../repositories/study_repository");
const habit_repository_1 = require("../repositories/habit_repository");
...
async function runE2ETestSuite() {
    ...
    try {
        ...
        const status = atlas_pool_1.atlasPool.getStatus();
        ...
        if (user) {
            const twinContext = await user_repository_1.userRepository.getTwinContext(user._id);
            ...
            assert(twinContext?.digital_twin_state?.burnout_risk_cluster === enums_1.BurnoutRisk.LOW_RISK, 'Embedded AI digital twin state retrieved in zero hops');
        }
        ...
        if (user && user.active_goals.length > 0) {
            ...
            const archived = await user_repository_1.userRepository.archiveGoal(user._id, targetGoal.goal_id, enums_1.GoalStatus.COMPLETED, mongoose_1.default.Types.Decimal128.fromString('50000.00'), 'AI Retrospective: Exceptional savings discipline demonstrated!');
        ...
        if (user) {
            ...
            const cashflowPipeline = await finance_repository_1.financialRecordRepository.getMonthlyCashflowPipeline(user._id, startDate);
        ...
        if (user) {
            const featureMatrix = await study_repository_1.studyActivityRepository.extractRandomForestFeatureMatrix(user._id);
        ...
        if (user) {
            const todayLog = await habit_repository_1.habitTrackingRepository.upsertDailyHabitLog(user._id, {
                sleep_hours: mongoose_1.default.Types.Decimal128.fromString('7.2'),
                exercise_minutes: 60,
                water_intake_liters: mongoose_1.default.Types.Decimal128.fromString('3.5'),
                screen_time_hours: mongoose_1.default.Types.Decimal128.fromString('4.5'),
                burnout_risk_cluster: enums_1.BurnoutRisk.LOW_RISK,
            });
            ...
            try {
                await Models.HabitTracking.create({
                    user_id: user._id,
                    sleep_hours: mongoose_1.default.Types.Decimal128.fromString('5.0'),
                    exercise_minutes: 10,
                    water_intake_liters: mongoose_1.default.Types.Decimal128.fromString('1.0'),
                    screen_time_hours: mongoose_1.default.Types.Decimal128.fromString('12.0'),
                    burnout_risk_cluster: enums_1.BurnoutRisk.HIGH_RISK,
            ...
            const kmeansMatrix = await habit_repository_1.habitTrackingRepository.extractKMeansFeatureSpace(user._id, 30);
